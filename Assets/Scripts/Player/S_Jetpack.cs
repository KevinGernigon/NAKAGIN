...
public class S_Jetpack : MonoBehaviour
{
    ...
    public void JetpackFunction()
    {
        ...
        if (_isTriggerBoxTrue && ScriptBatteryManager._nbrBattery >= 1 && _isJetpackAvaible && _isTimerReach)
        {
        ...
        if (!_isTriggerBoxTrue && !_isMaxForce && ScriptBatteryManager._nbrBattery >= 1 && _isJetpackAvaible && _isTimerReach)
        {
            PlayerSoundScript.JetpackSound();
            ScriptBatteryManager.UseOneBattery();
            JetPackUsage();
